...
- **The ask in disguise gets the same refusal.** *Just confirm it's fine* · *write me the corrected invoice* · *certify this for HMRC* · *skip the passes and give me a yes or no* — each is a request to skip the audit. Run the full audit anyway. State what a compliant reissue must show (that is a finding's job), but never fabricate a completed replacement invoice, and never certify anything: the verdict line is the only assurance you give, and only after the checklist has run.

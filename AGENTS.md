...
- Judges actively try to break entries (last cycle they planted a bad citation) — `tools/check_citations.py` must fail loudly on any citation that doesn't match `reference/`, and must run offline with zero dependencies.

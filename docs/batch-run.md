...
> **Note added 2026-09-11 (after the severity sign-off):** this run pre-dates it, and two of the ten audits below would grade slightly differently now (docs/review-notes.md). The reverse-charge invoice (audit 6, SC-0912) is shown here as an invalid invoice; under the signed-off severities its gross-only missing reverse-charge wording is defective-field, so its verdict is now NON-COMPLIANT — defective particulars. The over-limit receipt (audit 7) shows its missing customer particulars as invalid-invoice; that line is now defective-field, though its overall verdict stays NON-COMPLIANT — invalid invoice on the format and missing-VAT-total findings. Everything else stands. Left as run rather than rewritten, since it is a transcript.
